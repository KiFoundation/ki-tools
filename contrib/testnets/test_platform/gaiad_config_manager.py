...
target_dir = working_directory + 'mytestnet/node0/kid/config/'
...
def get_validator_pubkey(target_dir):
    val_pub_key = subprocess.check_output(['kid', 'tendermint', 'show-validator', '--home', target_dir.rstrip('/config')])
...
def get_validator_id(target_dir):
    ...
    # subprocess.call(['kid', 'init', '--home', target_dir])
    nodeid = subprocess.check_output(['kid', 'tendermint', 'show-node-id', '--home', str(target_dir).rstrip("config/")])
...
if len(template_replacements['replacement_genesis']) > 0:
    ...
    if len(template_replacements['replacement_genesis_make_safe']) > 0:
        # kid testnet --keyring-backend test --v 4
        print("Creating testnet subdirectories")
        subprocess.call(['rm', '-rf', working_directory + 'mytestnet'])
        subprocess.call(['kid', 'testnet', '--keyring-backend', 'test', '--v', str(safe_index)])

    # specify the output
    for node_num in range(safe_index):
        target_file = target_dir.replace("node0", "node" + str(node_num))
        target_files.append(target_file)
        # subprocess.call("kid init node" + str(node_num) + " -o --home "+target_file.rstrip('/config'), shell=True)
        subprocess.call('kid unsafe-reset-all --home ' + target_file.rstrip('/config'), shell=True)
    ...
    cmd_string = 'kid migrate ' + common_genesis + ' --chain-id cosmoshub-4 --initial-height 0  --replacement-cons-keys ' + working_directory + 'templates/validator_replacement_output.json > ' + working_directory + 'templates/genesis_replaced.json'
    ...
    subprocess.call(['kid', 'testnet', '--keyring-backend', 'test', '--v', str(num_of_nodes_to_apply)])
...
# time.sleep(60 * 10)
# tendermint_validator_set = subprocess.check_output(['kid', 'query', 'tendermint-validator-set']).decode("utf-8").rstrip('\n')
...
for target_file in target_files:
    ...
    proc = subprocess.Popen(['kid', 'start', '--home', target_file.rstrip('/config'), '--x-crisis-skip-assert-invariants'])

    # automatically terminate program (and thus all kid instances) after some time
