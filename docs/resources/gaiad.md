...
`kid` is the tool that enables you to interact with the node that runs on the Cosmos Hub network, whether you run it yourself or not. Let us set it up properly. In order to install it, follow the [installation procedure](../getting-started/installation.md).

### Setting up kid

The main command used to set up `kid` is the following:
...
- `cosmos`
  - Derived from account keys generated by `kid keys add`
...
- `cosmosvalconspub`
  - Generated when the node is created with `kid init`.
  - Get this value with `kid tendermint show-validator`
...
Older versions of `kid` used store keys in the user's home directory. If you are migrating
from an old version of `kid` you will need to migrate your old keys into your operating system's
...
When broadcasting transactions, `kid` accepts a `--broadcast-mode` flag. This
...
```bash
kid tx bank send ... --fees=50000uatom
```

or

```bash
kid tx bank send ... --gas-prices=0.0025uatom
```
...
```bash
kid tx bank send <sender_key_name_or_address> <recipient_address> 10faucetToken \
  --chain-id=<chain_id>
```
...
```bash
kid tx bank send <sender_key_name_or_address> <destination_cosmosaccaddr> 10faucetToken \
  --chain-id=<chain_id> \
  --dry-run
```
...
```bash
kid tx bank send <sender_address> <recipient_address> 10faucetToken \
  --chain-id=<chain_id> \
...
::: tip Note
The `--generate-only` flag prevents `kid` from accessing the local keybase.
...
where `[name]` is the name of the key you specified when you initialized `kid`.
...
```bash
kid query distribution community-pool
```
...
```bash
kid tx bank send cosmos1570v2fq3twt0f0x02vhxpuzc9jc4yl30q2qned 1000000uatom \
