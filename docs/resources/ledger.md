...
Installing the `Cosmos` application on your ledger device is required before you can use either [Keplr](#keplr-+-ledger-nano) or [`kid`](#kitools-cli-+-ledger-nano). To do so, you need to:
...
The tool used to generate addresses and transactions on the Cosmos Hub network is `kid`. Here is how to get started. If using a CLI tool is unfamiliar to you, scroll down and follow instructions for using the Keplr wallet instead.
...
Verify that kid is installed correctly with the following command
...
- Create an account in kid from your ledger key.

::: tip
Be sure to change the _keyName_ parameter to be a meaningful name. The `ledger` flag tells `kid` to use your Ledger to seed the account.
:::

```bash
kid keys add <keyName> --ledger
...
```bash
kid keys add <secondKeyName> --ledger --account <i>
```
...
Next, you need to configure kid with the URL of a Cosmos full node and the appropriate `chain_id`. In this example we connect to the public load balanced full node operated by Chorus One on the `cosmoshub-2` chain. But you can point your `kid` to any Cosmos full node. Be sure that the `chain-id` is set to the same chain as the full node.

```bash
kid config node https://cosmos.chorus.one:26657
kid config chain_id cosmoshub-2
```
...
You are now ready to start signing and sending transactions. Send a transaction with kid using the `tx send` command.

``` bash
kid tx send --help # to see all available options.
```
...
Use the `keyName` you set for your Ledger key and kitools will connect with the Cosmos Ledger app to then sign your transaction.

```bash
kid tx send <keyName> <destinationAddress> <amount><denomination>
```
...
Not sure what `kid` can do? Simply run the command without arguments to output documentation for the commands in supports.

::: tip
The `kid` help commands are nested. So `$ kid` will output docs for the top level commands (status, config, query, and tx). You can access documentation for sub commands with further help commands.
...
- `chain-id`: The chain to which you are broadcasting the tx, such as the `kitools-13003` testnet or `cosmoshub-2`: mainnet.
