...
- [Command-Line Interface](#command-line-interface)
  ...
  - [Remote Access to kid](#remote-access-to-kid)
...
The command-line interface (CLI) is the most powerful tool to access the Cosmos Hub and use kitools.
To use the CLI, you must install the latest version of `kitools` on your machine.

Compare your version with the [latest release version](https://github.com/cosmos/kitools/releases)
...
All available CLI commands are shown when you run the `kid` command:
...
Flags:
  -h, --help                help for kid
      --home string         directory for config and data (default "/Users/tobias/.kitools")
      --log_format string   The logging format (json|plain) (default "plain")
      --log_level string    The logging level (trace|debug|info|warn|error|fatal|panic) (default "info")
      --trace               print out full stack trace on errors

Use "kid [command] --help" for more information about a command.
```
...
Use "kid query [command] --help" for more information about a command.
```

### Remote Access to kid

When choosing to remote access a Full Node and kid, you need a Full Node running and kitools installed on your local machine.

`kid` is the tool that enables you to interact with the node that runs on the Cosmos Hub network, whether you run it yourself or not.

To set up `kid` on a local machine and connect to an existing full node, use the following command:
...
```bash
kid config chain-id cosmoshub-4
```
...
The default key is `secp256k1 elliptic curve`. Use the `kid keys` command to list the keys and generate a new key.



```bash
kid keys add <your_key_name>
```
...
```bash
kid keys add <your_key_name> --recover
```
...
```bash
kid query account <YOUR_ADDRESS>
```
...
```bash
kid query bank balances <YOUR_ADDRESS>
```
...
```bash
kid tx send <from_key_or_address> <to_address> <amount> \
