...
만약 2월 18일 전에 업그레이드 과정을 미리 테스트 진행을 희망하시는 경우 [이 글](https://github.com/cosmos/kitools/issues/569#issuecomment-767910963)을 참고하세요
...
__이번 업그레이드에서 풀 노드 운영자 업그레이드를 진행하는 것은 [Gaia](https://github.com/cosmos/kitools) 애플리케이션 v4.0.0입니다. 이번 버전의 Gaia 애플리케이션은 코스모스 SDK v0.41.0 그리고 텐더민트 v0.34.3 기반으로 빌드되었습니다.
...
- **CLI**: 이전 버전의 코스모스 SDK에서는 블록체인의 CLI와 데몬은 별도의 바이너리로 구성되었으며, 실행하는 블록체인 인터랙션에 따라 `kid`와 `kitoolscli` 바이너리가 구분되었습니다. 이번 버전의 코스모스 SDK에서는 두 바이너리가 하나의 `kid` 바이너리로 통합되었으며 해당 바이너리 내에서 기존에 `kitoolscli`에서 사용했던 명령어를 지원합니다.
- **노드 구성**: 이전 버전의 코스모스 SDK에서는 블록체인 데이터와 노드 설정이 `~/.kitools/`에 저장되었지만, 이번 버전에서는 해당 정보다 `~/.kitools/` 디렉토리에 보관됩니다. 만약 블록체인 데이터 또는 노드 설정을 관리하는 스크립트를 사용하시는 경우 해당 스크립트에서 패스를 변경해야합니다.
...
각 검증인은 `cosmoshub-3` 상태를 내보내기(export) 전에 내보내는 블록 하이트의 풀 데이터 스냅샷을 진행할 것을 권장합니다. 스냅샷 과정은 각 검증인의 인프라에 따라 다를 수 있지만, 통상 `.kitools` 디렉토리를 백업하는 것으로 진행합니다.

`kid` 프로세스를 멈춘 후 `.kitools/data/priv_validator_state.json` 파일을 백업하는 것은 매우 중요합니다. 이 파일은 검증인이 컨센서스 라운드에 참여할 때마다 업데이트됩니다. 만약 업그레이드 과정이 실패하여 이전 체인을 다시 시작해야되는 경우 검증인의 이중서명을 방지하기 위해서 이 파일은 필수입니다.

만약 업그레이드 과정이 실패하는 경우, 검증인과 노드 운영자는 kitools v2.0.15(코스모스 SDK v0.37.15 기반)으로 다운그레이드를 진행하고 가장 최근 진행했던 스냅샷을 복구한 이후에 노드를 시작해야합니다.


## 업그레이드 

__참고__: 이 가이드는 코스모스 SDK의 v0.37.15 기반의 kitools v2.0.15를 운영한다는 가정에 작성된 가이드입니다.

Gaia v2.0.15의 버전/커밋 해시값: `89cf7e6fc166eaabf47ad2755c443d455feda02e`

1. 올바른 _kid_ 버전 (v2.0.15)를 운영하고 있는 것을 확인하세요:
...
1. 체인이 올바른 날짜와 시간에 멈추도록 설정하세요:
 ...
 1. 체인이 멈춘 후 `.kitools` 디렉토리를 백업하세요
...
1. 기존 `cosmoshub-3`의 상태를 내보내기:
   ...
   다음 명령어를 사용하여 상태를 내보내기 전 `kid` 바이너리가 꼭 멈춰있어야 합니다!
   검증인으로서 가장 최근 생성된 블록은 `~/.kitools/config/data/priv_validator_state.json`에서 확인하실 수 있습니다 (또는 이전 과정에서 백업을 진행한 경우 `kid_backup`). 블록 높이는 다음과 같이 확인하세요
...
1. 이 단계 까지 오셨다면 올바른 제네시스 상태를 내보내셨습니다! 이후 과정부터는 [Gaia](https://github.com/cosmos/kitools) v4.0.0을 필요로 합니다. 그룹 채팅 방의 다른 검증인들/피어와 새로운 제네시스 파일의 해시를 비교/검증하세요.
...
1. 새로운 `genesis.json`을 `.kitools/config/` 디렉토리로 옮기세요:
...
1. 블록체인을 가동하세요
    ...
    Crisis 모듈의 제네시스 상태 자동 검증은 30-120분 소요될 수 있습니다. 해당 기능은 `kid start --x-crisis-skip-assert-invariants`로 비활성화할 수 있습니다.

## 서비스 제공자를 위한 정보

# REST 서버

만약 이전까지 `kitoolscli rest-server` 명령어로 REST 서버를 구동하신 경우, 해당 명령어는 이번 버전부터 비활성화 됩니다. API 서버는 데몬 내에서 활성화되며, `.kitools/config/app.toml` 설정 내에서 활성화됩니다:
...
gRPC 설정은 `.kitools/config/app.toml`에 있습니다.
...
스테이트 싱크 설정은 `.kitools/config/app.toml`에 있습니다.
