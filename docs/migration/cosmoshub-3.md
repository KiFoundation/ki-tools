...
https://github.com/cosmos/kitools/issues/569#issuecomment-767910963
...
__[Gaia](https://github.com/cosmos/kitools) application v4.0.2 is
...
led to a `kid` and `kitoolscli` binary which were seperated and could be used for different interactions with the
blockchain. Both of these have been merged into one `kid` which now supports the commands the `kitoolscli` previously
supported.
- **Node Configuration**: Previously blockchain data and node configuration was stored in `~/.kitools/`, these will
now reside in `~/.kitools/`, if you use scripts that make use of the configuration or blockchain data, make sure to update the path.
...
can be done by backing up the `.kitools` directory.

It is critically important to back-up the `.kitools/data/priv_validator_state.json` file after stopping your kid process. This file is updated every block as your validator participates in a consensus rounds. It is a critical file needed to prevent double-signing, in case the upgrade fails and the previous chain needs to be restarted.

In the event that the upgrade does not succeed, validators and operators must downgrade back to
kitools v2.0.15 with v0.37.15 of the _Cosmos SDK_ and restore to their latest snapshot before restarting their nodes.

## Upgrade Procedure

__Note__: It is assumed you are currently operating a full-node running kitools v2.0.15 with v0.37.15 of the _Cosmos SDK_.

The version/commit hash of Gaia v2.0.15: `89cf7e6fc166eaabf47ad2755c443d455feda02e`

1. Verify you are currently running the correct version (v2.0.15) of _kid_:
...
1. Make sure your chain halts at the right time and date:
 ...
 1. After the chain has halted, make a backup of your `.kitools` directory
...
1. Export existing state from `cosmoshub-3`:
   ...
   Before exporting state via the following command, the `kid` binary must be stopped!
   As a validator, you can see the last block height created in the 
   `~/.kitools/data/priv_validator_state.json` - or now residing in `kid_backup` when you made
    a backup as in the last step - and obtain it with

   ```bash
   cat ~/.kitools/data/priv_validator_state.json | jq '.height'
   ```

   ```bash
   $ kid export --height=<height> > cosmoshub_3_genesis_export.json
   ```
...
v4.0.2 of [Gaia](https://github.com/cosmos/kitools). 
Cross check your genesis hash with other peers (other validators) in the chat rooms.
   ...
   **NOTE**: Go [1.15+](https://golang.org/dl/) is required!
   ...
   ```bash
   $ git clone https://github.com/cosmos/kitools.git && cd kitools && git checkout v4.0.2; make install
   ```
...
1. Move the new `genesis.json` to your `.kitools/config/` directory
...
1. After the chain has halted, make a backup of your `.kitools` directory
   ...
   **NOTE**: It is recommended for validators and operators to take a full data snapshot at the export
   height before proceeding in case the upgrade does not go as planned or if not enough voting power
   comes online in a sufficient and agreed upon amount of time. That means the backup of `.kitools` should 
...
1. Install v4.0.2 of [Gaia](https://github.com/cosmos/kitools).
...
In case you have been running REST server with the command `kitoolscli rest-server` previously, running this command will not be necessary anymore.
API server is now in-process with daemon and can be enabled/disabled by API configuration in your `.kitools/config/app.toml`:
...
gRPC configuration in your `.kitools/config/app.toml`
...
State Sync Configuration in your `.kitools/config/app.toml`
