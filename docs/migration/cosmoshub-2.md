...
[Gaia](https://github.com/cosmos/kitools) application have been split into separate
repositories. This allows for both the Cosmos SDK and Gaia to evolve naturally
and independently. Thus, any future [releases](https://github.com/cosmos/kitools/releases)
...
on its current versioning path (i.e. v0.36.x ) and the [Gaia](https://github.com/cosmos/kitools)
application will become v2.0.x.

__[Gaia](https://github.com/cosmos/kitools) application v2.0.3 is
...
can be done by backing up the `.kitools` directories.
...
2. Export existing state from `cosmoshub-2`:
   ...
   **NOTE**: It is recommended for validators and operators to take a full data snapshot at the export
   ...
   ```bash
   $ kid export --for-zero-height --height=2902000 > cosmoshub_2_genesis_export.json
   ```
...
v2.0.3 of [Gaia](https://github.com/cosmos/kitools).

   **NOTE**: Go [1.13+](https://golang.org/dl/) is required!

   ```bash
   $ git clone https://github.com/cosmos/kitools.git && cd kitools && git checkout v2.0.3; make install
   ```
...
6. Migrate exported state from the current v0.34.6+ version to the new v2.0.3 version:
   ...
   ```bash
   $ kid migrate v0.36 cosmoshub_2_genesis_export.json --chain-id=cosmoshub-3 --genesis-time=[PLACEHOLDER]> genesis.json
   ```
...
10. Move the new `genesis.json` to your `.kitools/config/` directory
11. Replace the `db_backend` on `.kitools/config/config.toml` to:

    ```toml
    db_backend = "goleveldb"
    ```

12. Note, if you have any application configuration in `kid.toml`, that file has now been renamed to `app.toml`:

    ```bash
    $ mv .kitools/config/kid.toml .kitools/config/app.toml
    ```
...
4. We highly recommend standing up a [testnet](https://github.com/cosmos/kitools/blob/master/docs/deploy-testnet.md)
   with the `kitools-2.0` release or joining the kitools-13006 testnet. More info for joining the testnet can be
