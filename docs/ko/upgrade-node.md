...
이 문서는 `kid` 풀노드를 새로운 버전으로 업그레이드하는 절차를 설명합니다.

## 소프트웨어 업그레이드

우선 `kid` 인스턴스를 중지하세요. 이후 소프트웨어를 업그레이드하세요:
...
```bash
cd $HOME/.kitools/config
kid export --for-zero-height --height=<내보내는_블록_높이(export-height)> > new_genesis.json
```
...
::: warning
메인넷에서 **검증인 노드**를 운영하시는 경우, `kid unsafe-reset-all` 명령어를 실행하실때 주의를 기울이세요. `chain-id`를 변경하지 않는 이상 이 명령어를 실행할 필요가 없습니다.
:::
