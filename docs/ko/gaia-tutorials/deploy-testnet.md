...
해당 문서는 `kid` 노드 네트워크를 구축하는 세가지 방법을 설명합니다. 각 테스트넷 모델은 다른 테스트 목적에 최적화 되어있습니다.
...
# genesis.app_state.staking.params.bond_denom의 기본 설정은 is staking kid add-genesis-account $(kid keys show validator -a) 1000stake,1000validatortoken 입니다.
...
# 이제 `kid`를 실행하실 수 있습니다.
kid start
```

이 셋업은 모든 `kid` 정보를 `~/.kitools`에 저장힙니다. 생성하신 제네시스 파일을 확인하고 싶으시다면 `~/.kitools/config/genesis.json`에서 확인이 가능합니다. 위의 세팅으로 `kid`가 이용이 가능하며, 토큰(스테이킹/커스텀)이 있는 계정 또한 함께 생성됩니다.
...
`localnet` 커맨드를 운영하기 위한 `kid` 바이너리(리눅스)와 `tendermint/kidnode` docker 이미지를 생성합니다. 해당 바이너리는 컨테이너에 마운팅 되며 업데이트를 통해 이미지를 리빌드 하실 수 있습니다.
...
이 커맨드는 4개 노드로 구성되어있는 네트워크를 kidnode 이미지를 기반으로 생성합니다. 각 노드의 포트는 하단 테이블에서 확인하실 수 있습니다:
...
`make localnet-start`는 `kid testnet` 명령을 호출하여 4개 노드로 구성된 테스트넷에 필요한 파일을 `./build`에 저장합니다. 이 명령은 `./build` 디렉토리에 다수의 파일을 내보냅니다.
...
각 `./build/nodeN` 디렉토리는 각자 컨테이너 안에 있는 `/kid`에 마운팅 됩니다.

### 로깅

로그는 각 `./build/nodeN/kid/kitools.log`에 저장됩니다. 로그는 docker를 통해서 바로 확인하실 수도 있습니다:
...
`kid`를 이용해 tx를 생성하거나 상태를 쿼리 하시려면, 특정 노드의 `kid` 디렉토리를 `home`처럼 이용하시면 됩니다. 예를들어:
...
::: tip
**참고**: 각 노드의 시드는 `./build/nodeN/kid/key_seed.json`에서 확인이 가능하며 `kid keys add --restore` 명령을 통해 CLI로 복원될 수 있습니다.
:::
