...
::: warning
**여기에서 더 진행하시기 전에 [kitools](./installation.md)가 꼭 설치되어있어야 합니다.**
:::
...
`moniker`는 `~/.kitools/config/config.toml` 파일을 통해 추후에 변경이 가능합니다:
...
최소 가스 가격보다 낮은 트랜잭션을 거절하는 스팸 방지 메커니즘을 활성화 하시려면 `~/.kitools/config/kid.toml` 파일을 변경하시면 됩니다:
...
테스트넷의 `genesis.json`파일을 `kid`의 config 디렉토리로 가져옵니다.
...
이제 노드가 다른 피어들을 찾는 방법을 알아야합니다. `$HOME/.kitools/config/config.toml`에 안정적인 시드 노드들을 추가할 차례입니다. [`launch`](https://github.com/cosmos/launch) repo에 몇 개 시드 노드 링크가 포함되어있습니다.
...
풀노드는 컨펌되지 않은 트랜잭션을 멤풀에 보관합니다. 스팸 트랜잭션으로부터 풀노드를 보호하기 위해서 노드 멤풀에 보관되기 위한 트랜잭션의 최소 가스 가격(`minimum-gas-prices`)을 설정할 것을 권장합니다. 해당 파라미터는 `~/.kitools/config/kid.toml`에서 설정하실 수 있씁니다.
