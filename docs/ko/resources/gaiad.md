...
`kid`는 사용자가 풀노드를 통해 코스모스 허브 네트워크를 사용할 수 있게하는 도구입니다. 풀노드는 사용자가 직접 운영하거나 원격 풀노드에 연결하여 사용하실 수 있습니다. `kid`를 설치하기 위해서는 [다음 과정](./installation.md)을 참고하세요.

### kid 설정하기

`kid`를 설정하는데 사용되는 주 명령어는 다음과 같습니다:

```bash
kid config <플래그(flag)> <값(value)>
```
...
```bash
kid config node <호스트(host)>:<포트(port)>

# 예시: kid config node https://77.87.106.33:26657
```
...
- `cosmosvalconspub`
  - `kid init`로 새로운 노드가 생성될때 같이 생성되는 키.
  - `kid tendermint show-validator` 명령으로 키 값을 확인할 수 있음
...
```bash
kid keys add <계정_명칭(account_name)>
```
...
```bash
kid keys show <계정_명칭(account_name)>
```
...
트랜잭션을 전파할때 `kid`에서 `--broadcast-mode`를 사용하실 수 있습니다. `--broadcast-mode`는 `sync`(동기, 기본 값), `async`(비동기), 또는 `block`(보류) 값으로 설정될 수 있습니다. `sync` 값은 클라이언트가 CheckTx 리턴 값을 돌려주고, `async` 값은 클라이언트가 즉시 반응을 알려주며, `block` 값은 트랜잭션이 커밋(또는 타임이웃)될 때까지 대기합니다.
...
```bash
kid query account <코스모스_주소(account_cosmos)>
```
...
```bash
kid tx send <보내는_사람_키_명칭_또는_주소(sender_key_name_or_address)> 10faucetToken \
...
```bash
kid query account <보내는_주소(account_cosmos)>
kid query account <수신자_주소(destination_cosmos)>
```

특정 블록 높의에서의 잔고를 확인하고 싶으시다면 `--block` 플래그를 사용하실 수 있습니다:

```bash
kid query account <코스모스_주소(account_cosmos)> --block=<블록_높이(block_height)>
```
...
```bash
kid tx send <보내는이_주소(sender_address)> <수신자_주소(destination_cosmosaccaddr)> 10faucetToken \
  --chain-id=<체인_아이디(chain-id)> \
  --generate-only > unsignedSendTx.json
```

::: tip 참고
`--generate-only` 명령어는 `kid`가 로컬 키베이스를 액세스하지 않습니다. `--generate-only` 플래그를 사용하시는 경우, `<보내는_사람_키_명칭_또는_주소(sender_key_name_or_address)>` 값은 키 명칭이 아닌 주소 값을 입력하세요.
:::
...
```bash
kid tx sign --validate-signatures
  signedSendTx.json
```
...
```bash
kid tx slashing unjail --from <검증인_오퍼레이터_주소(validator-operator-addr)>
```
...
```bash
kid query slashing signing-info <검증인_pubkey(validator-pubkey)>
```
...
```bash
kid query minting params
```

현재 인플레이션 값은 다음과 같이 조회하실 수 있습니다:

```bash
kid query minting inflation
```

현재 프로비젼(provisions) 값은 다음과 같이 조회하실 수 있습니다:

```bash
kid query minting annual-provisions
```
...
```bash
kid query staking validator <cosmosval_계정(account_cosmosval)>
```
...
여기에서`[name]`은 `kid`를 처음 설정하셨을때 정의한 키의 명칭입니다.
...
```bash
kid query staking delegation <위임자_코스모스_주소(delegator_addr)> <검증인_주소(validator_addr)>
...
```bash
kid query staking delegation <위임자_코스모스_주소(delegator_addr)>
```
...
```bash
kid query staking unbonding-delegation <위임자_주소(delegator-addr)> <검증인_주소(validator_addr)>
```

또는 특정 위임자의 모든 언본딩 정보를 확인하고 싶으신 경우:

```bash
kid query staking unbonding-delegations <위임자_주소(account_cosmos)>
```

추가적으로 특정 검증인으로 부터 언본딩하는 정보를 확인하고 싶으신 경우:

```bash
kid query staking unbonding-delegations-from <검증인_cosmosval_주소(account_cosmosval)>
```
...
```bash
kid query staking redelegation <위임자_주소(delegator_addr)> <기존_검증인_주소(src_val_addr)> <대상_검증인_주소(dst_val_addr)>
```

특정 위임자의 모든 검증인에 대한 재위임을 확인하고 싶으신 경우:

```bash
kid query staking redelegations <위임자_주소(account_cosmos)>
```

특정 검증인에 대한 재위임을 확인하고 싶으신 경우:

```bash
  kid query staking redelegations-from <검증인_주소(account_cosmosval)>
```
...
```bash
  kid query delegations-to <검증인_주소(account_cosmosval)>
```
...
```bash
kid tx gov submit-proposal param-change <프로포절/파일/경로/프로포절.json> \
  --from=<키_명칭(key_name)> \
  --chain-id=<체인_아이디(chain_id)>
```
...
```bash
kid query gov proposal <프로포절_ID(proposal_id)>
```
...
```bash
kid query gov proposer <프로포절_ID(proposal_id)>
```
...
```bash
kid tx gov deposit <프로포절_ID(proposal_id)> "10000000uatom" \
  --from=<키_명칭(name)> \
...
```bash
kid query gov deposits <프로포절_ID(proposal_id)>
```

특정 주소에 대한 보증금은 다음과 같이 확인하실 수 있습니다:

```bash
kid query gov deposit <프로포절_ID(proposal_id)> <보증금_제공자_주소(depositor_address)>
```
...
```bash
kid tx gov vote <프로포절_ID(proposal_id)> <Yes/No/NoWithVeto/Abstain(표 선택)> \
  --from=<키_명칭(key_name)> \
  --chain-id=<체인_아이디(chain-id)>
```
...
```bash
kid query gov vote <프로포절_ID(proposal_id)> <투표자_주소(voter_address)>
```
과거 프로포절에 대한 표 정보를 확인하기 위해서는:

```bash
kid query gov votes <프로포절_ID(proposal_id)>
```
...
```bash
kid query gov tally <프로포절_ID(proposal_id)>
```
...
```bash
kid query distribution commission <검증인_주소(validator_address)>
```
...
```bash
kid query distribution slashes <검증인_주소(validator_address)> <시작_블록_높이(start_height)> <끝_블록_높이(end_height)>
```
...
```bash
kid query distribution rewards <위임자_주소(delegator_address)> <검증인_주소(validator_address)>
```
...
```bash
kid query distribution rewards <위임자_주소(delegator_address)>
```
...
흔히 사용되는 `Bash`와 `Zsh` 같은 UNIX의 완료 스크립트(completion script)는 `completion` 명령어를 사용해 생성될 수 있습니다. 이 명령은 `kid`와 `kid`에서 사용 가능합니다.
