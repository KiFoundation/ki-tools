...
1. Start the network and trigger upgrade
   ...
   ```bash
   # start a kitools application full-node
   $ kid start

   # set up the cli config
   $ kid config trust-node true
   $ kid config chain-id testing

   # create an upgrade governance proposal
   $ kid tx gov submit-proposal software-upgrade <plan-name> \
...
2. Performing an upgrade
   ...
   manually kill the process and replace it with a new binary. Do so now with `Ctrl+C` or `killall kid`.

   In `kitools/app/app.go`, after `upgrade.Keeper` is initialized and set in the app, set the the
   ...
   ```bash
   # create a new binary of kitools with the added upgrade handler
   $ make install
   ...
   $ kid query upgrade applied <plan-name>
   ```
