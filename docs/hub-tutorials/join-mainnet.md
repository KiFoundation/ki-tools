...
The current Cosmos Hub mainnet, `cosmoshub-4`, has been performing in place store migration upgrades as of the [Delta Upgrade](https://github.com/cosmos/kitools/blob/main/docs/migration/cosmoshub-4-delta-upgrade.md) July 2021. The most recent upgrade was [Vega](https://github.com/cosmos/kitools/blob/main/docs/migration/cosmoshub-4-vega-upgrade.md) December 2021. This type of upgrade preserves the same chain-id but state before the upgrade height is only accessible by corresponding versions of the binary (ie. queries of state between height `6910000` and `8695000` should use `kitools v5.0.x` (Delta) but after `86950000` should use `kitools v6.0.x` (Vega) to guarantee correctly encoded responses). Visit the [migration section](https://github.com/cosmos/kitools/tree/main/docs/migration) of the Hub's docs for more information on previous chain migrations.
...
The current Cosmos Hub mainnet `cosmoshub-4`. Visit the [migration section](https://github.com/cosmos/kitools/tree/main/docs/migration) of the Hub's docs for more information on previous chain migrations.
...
Make sure to walk through the basic setup and configuration. Operators will need to initialize `kid`, download the genesis file for `cosmoshub-4`, and set persistent peers and/or seeds for startup.

### Initialize Chain

Choose a custom moniker for the node and initialize. By default, the `init` command creates the `~/.kitools` directory with subfolders `config` and `data`. In the `/config` directory, the most important files for configuration are `app.toml` and `config.toml`.
```bash
kid init <custom-moniker>
```
...
The `moniker` can be edited in the `~/.kitools/config/config.toml` file:
...
mv genesis.cosmoshub-4.json ~/.kitools/config/genesis.json
```

### Seeds & Peers

Upon startup the node will need to connect to peers. If there are specific nodes a node operator is interested in setting as seeds or as persistent peers, this can be configured in `~/.kitools/config/config.toml`
...
Node operators can optionally download the [Quicksync address book]( https://quicksync.io/addrbook.cosmos.json). Make sure to move this to `~/.kitools/config/addrbook.json`.
...
A full-node keeps unconfirmed transactions in its mempool. In order to protect it from spam, it is better to set a `minimum-gas-prices` that the transaction must meet in order to be accepted in the node's mempool. This parameter can be set in  `~/.kitools/config/app.toml`.
...
To set pruning, adjust the `pruning` parameter in the `~/.kitools/config/app.toml` file.
...
Passing a flag when starting `kitools` will always override settings in the `app.toml` file. To change the node's pruning setting to `everything` mode then pass the `---pruning everything` flag when running `kid start`.
...
By default, the REST API is disabled. To enable the REST API, edit the `~/.kitools/config/app.toml` file, and set `enable` to `true` in the `[api]` section.
...
By default, gRPC is enabled on port `9090`. The `~/.kitools/config/app.toml` file is where changes can be made in the gRPC section. To disable the gRPC endpoint, set `enable` to `false`. To change the port, use the `address` parameter.
...
It is possible to sync from previous versions of the Cosmos Hub. See the matrix below for the correct `kitools` version. See the [mainnet archive](https://github.com/cosmos/mainnet) for historical genesis files.
...
```bash
kid start --x-crisis-skip-assert-invariants
...
Once state sync successfully completes, the node will begin to process blocks normally. If state sync fails and the node operator encounters the following error:  `State sync failed err="state sync aborted"`, either try restarting `kid` or running `kid unsafe-reset-all` (make sure to backup any configuration and history before doing this).
::::::
...
While not advised, if a node operator needs to customize this feature, it can be configured in `~/.kitools/config/app.toml`. The Cosmos Hub recommends setting this value to match `pruning-keep-every` in `config.toml`.
...
## Releases & Upgrades
**See all [Gaia Releases](https://github.com/cosmos/kitools/releases)**

The most up to date release of Gaia is [`V6.0.0`](https://github.com/cosmos/kitools/releases/tag/v6.0.0). For those that want to use state sync or quicksync to get their node up to speed, starting with the most recent version of Gaia is sufficient.

To sync an archive or full node from scratch, it is important to note that you must start with [`V4.2.1`](https://github.com/cosmos/kitools/releases/tag/v4.2.1) and proceed through two different upgrades Delta at block height `6910000` and Vega at block height `8695000`.

The process is summarized below but make sure to follow the manual upgrade instructions for each release:

**[Delta Instructions](https://github.com/cosmos/kitools/blob/main/docs/migration/cosmoshub-4-delta-upgrade.md#Upgrade-will-take-place-July-12,-2021)**
...
Make sure to save a backup of `~/.kitools` in case rolling back is necessary.

Install Gaia [`V5.0.0`](https://github.com/cosmos/kitools/releases/tag/v5.0.0) and restart the daemon.


**[Vega Instructions](https://github.com/cosmos/kitools/blob/main/docs/migration/cosmoshub-4-vega-upgrade.md)**
...
Again, make sure to backup `~/.kitools`

Install Gaia [`V6.0.0`](https://github.com/cosmos/kitools/releases/tag/v6.0.0) and restart the daemon.
...
```bash
Environment="DAEMON_HOME=$HOME/.kitools"
Environment="DAEMON_NAME=kid"
Environment="DAEMON_ALLOW_DOWNLOAD_BINARIES=false"
Environment="DAEMON_RESTART_AFTER_UPGRADE=true"
```

After the `LimitNOFILE` line and replace `$(which kid)` with `$(which cosmovisor)`.
...
```bash
kid start --assert-invariants-blockly
```
...
invariant broken:
    ...
    CRITICAL please submit the following transaction:
        kid tx crisis invariant-broken staking supply
