...
echo "export DAEMON_NAME=kid" >> ~/.profile
echo "export DAEMON_HOME=$HOME/.kitools" >> ~/.profile
source ~/.profile
```
...
```bash
mkdir -p ~/.kitools/cosmovisor/upgrades
mkdir -p ~/.kitools/cosmovisor/genesis/bin/
cp $(which kid) ~/.kitools/cosmovisor/genesis/bin/

# verify the setup. 
# It should return the same version as kid
cosmovisor version
```

Now `kid` can start by running
...
First, stop your instance of `kid`. Next, upgrade the software:
...
```bash
cd $HOME/.kitools/config
kid export --for-zero-height --height=<export-height> > new_genesis.json
```
...
::: warning
If you are running a **validator node** on the mainnet, always be careful when doing `kid unsafe-reset-all`. You should never use this command if you are not switching `chain-id`.
:::
