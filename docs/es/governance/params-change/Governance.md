...
Esto es útil si no tienes `kid` instalado y no tienes una razón para creer que el parámetro ha cambiado desde que se lanzó la cadena.
...
## Parámetros actuales
Puede verificar los valores actuales de los parámetros (en caso de que hayan sido modificados mediante la propuesta de gobierno posterior al lanzamiento) con la aplicación de [línea de comandos kid](params-change/kid). Aquí están los comandos para cada uno:
1. `depositparams` - `kid q ..` --> **to do** <--
