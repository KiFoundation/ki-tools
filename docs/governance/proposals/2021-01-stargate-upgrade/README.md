...
------------
If passed, this governance proposal would commit the Cosmos Hub to halting the `cosmoshub-3` at 06:00 UTC on Jan 28th exporting the state and starting `cosmoshub-4` based on kitools 3.0.
...
This section is with the current `kitools 2.0.*` implementation.
  ...
  1. Validators should set their `kitools` with a halt time of `1611813600`ie. 06:00 UTC on Jan 28th in Unix time.
...
This section is with the upgraded `kitools 3.0.*` implemenataion.
...
1. A critical vulnerability may be found in the software. If the development teams change their recommended version of kitools, the validator set should implicitly abandon this upgrade procedure. A future proposal will be made to the Hub to upgrade to the new target commit.
