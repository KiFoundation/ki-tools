...
------------
If passed, this governance proposal would commit the Cosmos Hub to halting the `cosmoshub-3` at 06:00 UTC on Feb 18th exporting the state and starting `cosmoshub-4` based on kitools 4.0.0
...
* Bugs have been identified in the Proposal 29 implementation.  They are resolved in this pull request[Additional review of prop 29 and migration testing by zmanian · Pull Request #559 · cosmos/kitools · GitHub](https://github.com/cosmos/kitools/pull/559)
...
This upgrade also implements the fund recovery procedure defined in proposal 29. The code is [here](https://github.com/cosmos/kitools/blob/main/app/prop29.go). As requested during the discussion of the governance proposal, the migration and fund recovery procedure verifies all signatures. This code was tested during the cosmoshub-3 simulated upgrade testnet.
...
The upgrade instructions will continue to improve as a living [document](https://github.com/cosmos/kitools/blob/main/docs/migration/cosmoshub-3.md). The Gaia maintainers will continue to improve the instruction below.

This section is with the current `kitools 2.0.*` implementation.

  1. Validators should set their `kitools` with a halt time of `1613628000`ie. 06:00 UTC on Feb 18th in Unix time.

  2. Validators should then export the current cosmos state with `kid export > cosmoshub-3-export.json`

  3. Validators should determine the height of the last block as `last-cosmoshub-3_block`. This can be easily determined from kitools logs.

  4. Validators should back up their `.kid` directory.

This section is with the upgraded `kitools 4.0.*` implemenataion.

  1. Validators should then migrate the exported genesis state. `kid migrate cosmoshub-3-export.json --chain-id=cosmoshub-4 --initial-height [last_cosmoshub-3_block+1] > genesis.json`
  2. Validators should (**AFTER ENSURING IT IS BACKED UP**!) delete their `~/.kid` directory and create a new one with `kid init [name]` and then edit their configuration files or recover the config files from backups.
  3. Validators should then start `cosmoshub-4` with `kid start`. Automated audits of the genesis state can take 30-120 min using the crisis module. This can be disabled by `kid start --x-crisis-skip-assert-invariants`.
...
1. A critical vulnerability may be found in the software. If the development teams change their recommended version of kitools, the validator set should implicitly abandon this upgrade procedure. A signaling proposal will be made to indicate a vulnerability or defect in the upgrade has been found. A future proposal will be made to the Hub to upgrade to the new target commit.
