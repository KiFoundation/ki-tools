...
For information on how to use kid (the command line interface) to submit an on-chain proposal through the governance module, please refer to the [kid resource](../resources/kid.md) for the Cosmos Hub documentation.

### Walkthrough example

This is the command format for using kid (the command-line interface) to submit your proposal on-chain:
...
`kid tx gov submit-proposal param-change param.json --from gavin --chain-id kitools-13007 --node 45.77.218.219:26657`

This is the complete command that I could use to submit a **mainnet** parameter-change proposal right now:
`kid tx gov submit-proposal param-change param.json --from gavin --gas 500000 --fees 7500uatom --chain-id cosmoshub-3 --node cosmos-node-1.figment.network:26657`

1. `kid` is the command-line interface client that is used to send transactions and query the Cosmos Hub
...
6. `--chain-id cosmoshub-3` is Cosmos Hub 3. For current and past chain-id's, please look at the [cosmos/mainnet resource](https://github.com/cosmos/mainnet)
   - the testnet chain ID is [kitools-13007](https://hubble.figment.network/cosmos/chains/kitools-13007). For current and past testnet information, please look at the [testnet repository](https://github.com/cosmos/testnets)
...
```
kid q gov proposals --depositor cosmos1hxv7mpztvln45eghez6evw2ypcw4vjmsmr8cdx
```

Once you have the proposal ID, this is the command to deposit extra tokens:

```
kid tx gov deposit <proposal-id> <deposit> --from <name>
```
