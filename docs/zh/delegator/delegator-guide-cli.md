...
`kid`: 与`kid`全节点交互的命令行用户界面。 

::: 安全提示

**请检查并且确认你下载的`kid`是可获得的最新稳定版本**
:::
...
[**通过源代码安装**](https://cosmos.network/docs/kitools/installation.html)

::: tip 提示 

`kid` 需要通过操作系统的终端窗口使用，打开步骤如下所示： 
...
前，您需要先安装`kid`，同时，您需要知道你希望在哪里保存和使用您的私钥。 最好的办法是把他们保存在一台没有上网的电脑或者一个硬件钱包设备里面。 将私钥保存在一台联网的电脑里面比较危险，任何人通过网络攻击都有可能获取您的私钥，进而盗取您的资产。 
...
当您初始化钱包设备时，设备会产生一个24个词的助记词组。这个助记词组和Cosmos是兼容的，我们可以通过这个助记词组创建Cosmos账户。所以，您需要做的是确认您的钱包设备兼容`kid`，通过下面的步骤可以帮助您确认您的设备是否兼容：
...
这是最安全的途径，但需要相当多的资源。您需要有较大的带宽和至少1TB的磁盘容量来运行一个全节点。  
 ...
 `kitools`的安装教程在[这里](https://cosmos.network/docs/kitools/installation.html)， 如何运行一个全节点指导在[这里](https://cosmos.network/docs/kitools/join-testnet.html)
...
**在开始设置 `kid`前，请确认你已经可以[访问Cosmos Hub网络](#访问cosmos-hub网络)**
:::

::: 警告

**请确认您使用的`kid`是最新的稳定版本**
:::

无论您是否在自己运行一个节点，`kid` 都可以帮您实现与Cosmos Hub网络的交互。让我们来完成对它的配置。 


您需要用下面的命令行完成对`kid`的配置：
...
// 样例: kid config node https://77.87.106.33:26657
```
...
```bash
kid config chain-id gos-6
```

## 状态查询

::: 提示
** 准备抵押ATOM通证和取回奖励前，需要先完成 [`kid` 配置](#设置-kid)**
:::


`kid` 可以帮助您获得所有区块链的相关信息， 比如账户余额，抵押通证数量，奖励，治理提案以及其他信息。下面是一组用于委托操作的命令
...
::: 提示
**在您抵押通证或取回奖励之前，您需要完成[`kid` 设置](#设置-kid) 和 [创建账户](#创建一个账户)**
:::
...
kid tx staking delegate <validatorAddress> <amountToBond> --from <delegatorKeyName> --gas auto --gas-prices <gasPrice>


// 提取所有的奖励
// 参数设定样例: <gasPrice>=1000uatom

kid tx distribution withdraw-all-rewards --from <delegatorKeyName> --gas auto --gas-prices <gasPrice>
...
kid tx staking unbond <validatorAddress> <amountToUnbond> --from <delegatorKeyName> --gas auto --gas-prices <gasPrice>
```
...
kid tx gov submit-proposal --title "Test Proposal" --description "My awesome proposal" --type <type> --deposit=10000000uatom --gas auto --gas-prices <gasPrice> --from <delegatorKeyName>

// 增加对提案的抵押
// Retrieve proposalID from $kid query gov proposals --status deposit_period
// 通过 $kid query gov proposals --status deposit_period 命令获得 `proposalID` 
// 参数设定样例: <deposit>=1000000uatom

kid tx gov deposit <proposalID> <deposit> --gas auto --gas-prices <gasPrice> --from <delegatorKeyName>
...
kid tx gov vote <proposalID> <option> --gas auto --gas-prices <gasPrice> --from <delegatorKeyName>
```
...
kid tx staking delegate <validatorAddress> <amountToBond> --from <delegatorKeyName> --gas auto --gas-prices <gasPrice> --generate-only > unsignedTX.json
```
...
```bash
kid tx sign unsignedTx.json --from-addr <delegatorAddr>> signedTx.json
```
