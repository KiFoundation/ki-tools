...
chain_id = "kitools-11001"
reconnect = true # true is the default
secret_key = "~/.tmkms/secret_connection.key"

[[providers.ledgertm]]
chain_ids = ["kitools-11001"]
```

- Edit `addr` to point to your `kid` instance.
- Adjust `chain-id` to match your `.kitools/config/config.toml` settings.
...
The last step is to retrieve the validator key that you will use in `kid`.
...
The KMS may complain that it cannot connect to kid. That is fine, we will fix it in the next section.
...
You need to enable KMS access by editing `.kitools/config/config.toml`. In this file, modify `priv_validator_laddr` to create a listening address/port or a unix socket in `kid`.
...
Let's assume that you have set up your validator account and called it `kmsval`. You can tell kid the key that we've got in the previous section.

```bash
kid gentx --name kmsval --pubkey {.ValidatorKey} 
```

Now start `kid`. You should see that the KMS connects and receives a signature request.
