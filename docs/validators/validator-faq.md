...
- **Tendermint key**: A unique key that is used to sign consensus votes.
  - It is associated with a public key `cosmosvalconspub` (To get this value, run `kid tendermint show-validator`)
  - It is generated when the node is created with `kid init`.
- **Application key**: This key is created from the `kid` binary and is used to sign transactions. Application keys are associated with a public key that is prefixed by `cosmospub` and an address that is prefixed by `cosmos`. 

The Tendermint key and the application key are derived from account keys that are generated by the `kid keys add` command.
